...
class SubType(models.Model):
    ...
    large_price = models.DecimalField(max_digits=7, decimal_places=2)
...
class Sub(models.Model):
    ...
    price = models.DecimalField(max_digits=7, decimal_places=2, default=0.00, editable=False)
...
class PlatterType(models.Model):
    name = models.CharField(max_length=60, unique=True)
    small_price = models.DecimalField(max_digits=7, decimal_places=2)
    large_price = models.DecimalField(max_digits=7, decimal_places=2)
...
class MakananType(models.Model):
    name = models.CharField(max_length=60, unique=True)
    small_price = models.DecimalField(max_digits=7, decimal_places=2)
    large_price = models.DecimalField(max_digits=7, decimal_places=2)
...
class Makanan(models.Model):
    in_order = models.ForeignKey(ProperOrder, on_delete=models.CASCADE, null=True, related_name="makanan")
    already_ordered = models.BooleanField(default=False)
    add_by = models.ForeignKey(User, on_delete=models.CASCADE, null=True, blank=True)
    makanantype = models.ForeignKey(MakananType, on_delete=models.CASCADE)
    price = models.DecimalField(max_digits=7, decimal_places=0, default=0.00, editable=False)
    makanansize = models.CharField(choices=(("small", "small"), ("large", "large")), max_length=60, default="small")

    def calculate_price(self):
        print(self.plattersize)
        if self.makanansize == "small":
            self.price = self.makanantype.small_price
        elif self.makanansize == "large":
            self.price = self.makanantype.large_price
        # print(f"Price for {self.plattertype} version of Platter is {self.price}")

    def __str__(self):
        return f"Makanan: {str(self.makanantype)}"