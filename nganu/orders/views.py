...
def delete_all_user_orders(username):
    ...
    Makanan.objects.filter(add_by=username).filter(already_ordered=False).delete()
...
def calculate_cart_price(username):
    ...
    for obj in Platter.objects.filter(add_by=username).filter(already_ordered=False):
        price_all += obj.price
    for obj in Makanan.objects.filter(add_by=username).filter(already_ordered=False):
        price_all += obj.price
...
def cart_view(request):
    ...
    context.update({"Makanan": Makanan.objects.filter(add_by=request.user).filter(already_ordered=False)})
...
def menu_view(request):
    ...
    context.update({"Makanan": MakananType.objects.all()})
...
def makanan_view(request):
    ...
    context.update({"PizzaToppings": PizzaTopping.objects.all()})
    context.update({"form_pizza": form_pizza})
    return render(request, 'orders/makanan.html', context)
...
def minuman_view(request):
    ...
    context.update({"Sub": SubType.objects.all()})
    context.update({"Pasta": PastaType.objects.all()})
    context.update({"Minuman": SaladType.objects.all()})
    context.update({"Makanan": PlatterType.objects.all()})
    context.update({"Makanan": MakananType.objects.all()})
    context.update({"Pizza": PizzaType.objects.all()})
    context.update({"PizzaToppings": PizzaTopping.objects.all()})
    context.update({"form_pizza": form_pizza})
    return render(request, 'orders/minuman.html', context)
...
def add_to_cart(request, item_type, item_id, item_bigger, add_cheese):
    ...
    elif item_type == "makanan":
        item_id = MakananType.objects.get(id=item_id)
        new_makanan = Makanan(makanantype=item_id, makanansize=item_bigger, add_by=request.user)
        new_makanan.calculate_price()
        new_makanan.save()
        messages.add_message(request, messages.INFO, "Makanan added!")
...
def make_order(request):
    ...
    for item in Makanan.objects.filter(add_by=request.user).filter(already_ordered=False):
        item.already_ordered = True
        item.in_order = new_proper_order
        item.save()
